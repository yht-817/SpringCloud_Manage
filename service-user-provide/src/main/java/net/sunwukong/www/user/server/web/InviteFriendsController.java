...
public class InviteFriendsController extends BaseController {
    ...
    public ResponseData smsInvite(@RequestBody(required = false) RequestData<Map<String,String>> requestData){
        return iInviteFriendsService.smsInvite(requestData.getData());
    }

    @RequestMapping(value = "/acceptinvitation", produces = MediaType.APPLICATION_JSON_UTF8_VALUE, method = RequestMethod.POST)
    @ApiOperation(value = "分享接受邀请", response = ResponseData.class)
    @ApiImplicitParams({
    ...
    public ResponseData AcceptInvitation(@RequestBody(required = false) RequestData<Map<String, String>> requestData) {
        ...
        return iInviteFriendsService.acceptInvitation(data.get("userNo"),data.get("account"),data.get("accountType"));
    }
}
