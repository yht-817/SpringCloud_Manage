...
public class UserCityController extends BaseController {
    ...
    @ApiOperation(value = "根据用户编码获取用户服务城市", httpMethod = "POST", notes = "根据用户编码获取用户服务城市", response = ResponseData.class)
    @ApiImplicitParams({
    ...
    public ResponseData getUserCity(@RequestBody(required = false) RequestData<Map<String, String>> requestData) {
        return iUserCityService.getUserCity(requestData.getData().get("userNo"));
    }
}
