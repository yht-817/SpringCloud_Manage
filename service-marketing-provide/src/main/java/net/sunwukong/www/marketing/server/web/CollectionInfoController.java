...
public class CollectionInfoController extends BaseController {
    ...
    @ApiOperation(value = "获取用户收藏信息", httpMethod = "POST", notes = "获取用户收藏信息",response = ResponseData.class)
    @ApiImplicitParams({
    ...
    public ResponseData getUserCollectionInfoPage(@RequestBody(required = false) RequestData<Map<String,String>> requestData){
        ...
        return iCollectionInfoService.getUserCollectionInfoPage(data.get("userNo"),
                Integer.parseInt(data.get("pageNo")),
                Integer.parseInt(data.get("pageSize")),
    ...
    public ResponseData setUserCollection(@RequestBody(required = false) RequestData<Map<String,String>> requestData) {
        return iCollectionInfoService.setUserCollection(requestData.getData());
    }
}
